...
import type {
	CryptoLatestItem,
	GetCryptoLatestInput,
	GetCryptoLatestOutput,
	ListCryptoSourcesOutput,
	SearchCryptoLatestInput,
} from './schemas.js';
import {
...
const USER_AGENT =
	'ContinuumCryptoLatest/1.0 (+https://github.com/ContinuumDAO/continuum-node-sdk)';

export type CryptoLatestFetchDeps = {
...
function limitItems(items: CryptoLatestItem[], limit: number): CryptoLatestItem[] {
	return items
		.slice()
		.sort((a, b) => {
...
async function loadSources(
	sources: readonly CryptoLatestSource[],
	limit: number,
	fetchImpl: typeof fetch,
): Promise<GetCryptoLatestOutput> {
...
export function listCryptoSources(): SdkResult<ListCryptoSourcesOutput> {
	return {
		ok: true,
		data: {
			sources: CRYPTO_LATEST_SOURCES.map(source => ({
				id: source.id,
...
export async function getCryptoLatest(
	input: GetCryptoLatestInput,
	deps: CryptoLatestFetchDeps = {},
): Promise<SdkResult<GetCryptoLatestOutput>> {
	const resolved = resolveCryptoLatestSources(input.sourceId);
	if (!resolved.ok) {
		return resolved;
	}
...
export async function searchCryptoLatest(
	input: SearchCryptoLatestInput,
	deps: CryptoLatestFetchDeps = {},
): Promise<SdkResult<GetCryptoLatestOutput>> {
	const fetched = await getCryptoLatest({sourceId: input.sourceId, limit: 25}, deps);
	if (!fetched.ok) {
		return fetched;
	}
