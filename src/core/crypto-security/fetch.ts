...
import type {
	CryptoSecurityItem,
	GetCryptoSecurityLatestInput,
	GetCryptoSecurityLatestOutput,
	ListCryptoSecuritySourcesOutput,
	SearchCryptoSecurityInput,
} from './schemas.js';
import {
...
const USER_AGENT =
	'ContinuumCryptoSecurity/1.0 (+https://github.com/ContinuumDAO/continuum-node-sdk)';

export type CryptoSecurityFetchDeps = {
	fetchImpl?: typeof fetch;
};

type LoadedItem = CryptoSecurityItem & {searchText: string};
...
function toOutputItem(item: LoadedItem): CryptoSecurityItem {
	const {searchText: _searchText, ...rest} = item;
	return rest;
}

async function loadSources(
	sources: readonly CryptoSecuritySource[],
	limit: number,
	fetchImpl: typeof fetch,
): Promise<{items: LoadedItem[]; feeds: GetCryptoSecurityLatestOutput['feeds']}> {
...
export function listCryptoSecuritySources(): SdkResult<ListCryptoSecuritySourcesOutput> {
	return {
		ok: true,
		data: {
			sources: CRYPTO_SECURITY_SOURCES.map(source => ({
				id: source.id,
...
export async function getCryptoSecurityLatest(
	input: GetCryptoSecurityLatestInput,
	deps: CryptoSecurityFetchDeps = {},
): Promise<SdkResult<GetCryptoSecurityLatestOutput>> {
	const resolved = resolveCryptoSecuritySources(input.sourceId);
	if (!resolved.ok) {
		return resolved;
	}
...
export async function searchCryptoSecurity(
	input: SearchCryptoSecurityInput,
	deps: CryptoSecurityFetchDeps = {},
): Promise<SdkResult<GetCryptoSecurityLatestOutput>> {
	const resolved = resolveCryptoSecuritySources(input.sourceId);
	if (!resolved.ok) {
