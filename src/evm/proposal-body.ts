...
import {resolveChainRegistryEntry} from '../core/registry/networks-read.js';
...
export type BuiltMultiSignProposalBody = {
	readonly bodyForSign: Record<string, unknown>;
	readonly chainId: number;
	readonly isBatch: boolean;
};

/** Build multiSignRequest route fields only — browser-safe (no management-signer / node:fs). */
export async function buildMultiSignProposalBody(
	config: NodeSdkConfig,
	input: BuildMultiSignProposalInput,
): Promise<SdkResult<BuiltMultiSignProposalBody>> {
	...
	if (actions.length === 1) {
		const singleCalldata = encodeActionCalldata(
			actions[0]!.signature.trim(),
			actions[0]!.args,
		);
		const msgRaw =
			typeof singleCalldata === 'string' && singleCalldata.startsWith('0x')
				? singleCalldata.slice(2)
				: singleCalldata;
		bodyForSign = {
			keyList,
