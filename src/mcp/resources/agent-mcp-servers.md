...
Other slim toolsets on the same resolver (still no catalog dump): **`continuum-dao-compose`** (etherscan + foundry), **`block-explorer`** (pick etherscan or blockscout), **`dune-analytics`**, **`sec-filings`** (edgartools). **`agent-defaults`** lists recommended no-spam defaults; on a **new** conversation include **`firstReplyHint`** only (one sentence) — do not enumerate **`servers[]`** unless asked.
