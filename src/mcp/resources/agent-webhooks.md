...
Active webhooks live in MongoDB (`LocalAgentWebhooks`). Bundled templates are in `agent_llm_config.defaults/hooks/webhooks.json`.
...
2. **`list_bundled_webhook_templates`** — template names from mpc-config catalog.
3. **`add_webhook_from_catalog`** or **`add_webhook`** (custom) — creates job + auto-generated secret env var name.
