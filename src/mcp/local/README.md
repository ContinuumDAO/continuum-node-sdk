...
- **`push-image.sh`** — build context is the **parent directory** of `continuum-node-sdk/`. Runs **release preflight** (SDK lock, sibling defi dist) and **smoke test** before push — same checks as **`npm run release:mcp`**. Supports **`--build-only`**. Uses **`docker build --network=host`** on Linux so `npm ci` can reach the registry (bridge DNS often hangs ~10 min). Override: **`CONTINUUM_MCP_DOCKER_BUILD_NETWORK=default`**
- **Release from node-app** — **`npm run release:mcp`** wraps the same Dockerfile and shared **`scripts/mcp-release-lib.sh`** (thin orchestration from the app repo). See **`continuumdao-node-app/scripts/release/README.md`**.
