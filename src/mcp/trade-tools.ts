...
const buildTradeBaseSchema = z
	.object({
		...
		protocolId: z.enum(['hyperliquid', 'arcus', 'gmx', 'uniswap']),
		keyGenId: z.string().trim().min(1),
		ed25519KeyGenId: z.string().trim().min(1).optional(),
	...
	server.registerTool(
		...
		async input => {
			...
			const built = await buildTradeFromTradeIdea(config, defiContext, {
				...
				...(input.ed25519KeyGenId ? {ed25519KeyGenId: input.ed25519KeyGenId} : {}),
...
export function registerTradeTools(
	...
	server.registerTool(
		...
		{
			description:
				'List persisted trade ideas from analyze_* tools on the current chart dataset (bound tradeIdeas[]). Returns numbered menu rows (tradeIdeaNumber) with side, status, confidence, entry/target/invalidation, targetBasis and invalidationBasis (brief how each level was calculated), chartDataSource/chartInterval/chartBarCount, and measured-move % from entry. Required before synthesizing a conclusion or consensus across ideas — cite tradeIdeaNumber from items[] exactly. Use with build_trade_from_* to submit multisign.',
